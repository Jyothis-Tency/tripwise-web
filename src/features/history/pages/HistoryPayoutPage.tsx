...
function AgencyPayoutManager({
  ...
  const updateTripPayAmount = (tripId: string, val: string) => {
    setTripPayAmount((prev) => ({ ...prev, [tripId]: val }));
    setError(null);
  };
  const [savingTripId, setSavingTripId] = useState<string | null>(null);
  const [expandedTripId, setExpandedTripId] = useState<string | null>(null);
  const [loadingHistoryTripId, setLoadingHistoryTripId] = useState<
    string | null
  >(null);
              ...
              <table className="w-full text-left text-xs whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-2 py-2.5 font-semibold text-center text-slate-600">
                      Status
                    </th>
                    <th className="px-2 py-2.5 font-semibold text-right text-slate-600">
                      Agency profit
                      ...
                      <Fragment key={tripId}>
                        <tr className="group transition-colors hover:bg-slate-50">
                          ...
                          <td className="px-2 py-2.5 text-right font-medium text-slate-700">
                            {fmtCurrency(t.agencyCost || 0)}
                          </td>
                          <td className="px-2 py-2.5 text-right font-medium text-slate-700">
                            {fmtCurrency(t.cabCost || 0)}
                          </td>
                          <td className="px-2 py-2.5 text-right font-semibold text-emerald-600">
                            {fmtCurrency(totalPaidSoFar)}
                          </td>
                          <td className="px-2 py-2.5 text-right font-semibold text-amber-700">
                            {fmtCurrency(t.remainingAmount || 0)}
                          </td>
                              ...
                              <div className="m-3 mt-0 bg-white rounded-md shadow-sm border border-slate-200 p-3">
                                ...
                                {loadingHistoryTripId === tripId ? (
                                  ...
                                  <div className="text-[11px] italic text-slate-400">
                                    No payment records exist for this trip.
                                  </div>
                                ) : (
    ...
    <div className="flex h-full overflow-hidden relative">
      ...
      <div
        className={`flex-1 overflow-hidden transition-all ${
          selectedAgencyId ? "flex flex-col" : "hidden md:flex flex-col"
        }`}
      >
        {selectedAgency ? (
          ...
          <div className="flex h-full flex-col items-center justify-center bg-slate-50 p-6">
            <div className="flex max-w-sm flex-col items-center text-center p-6 border border-slate-200 bg-white rounded-lg shadow-sm">
              ...
              <p className="mt-2 text-xs text-slate-500">
                Choose an agency from the sidebar to view their payout history
                and manage payments.
              </p>
              <button
                type="button"
