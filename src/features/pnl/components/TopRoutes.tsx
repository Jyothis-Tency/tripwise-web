    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
      ...
      <div className="space-y-3 sm:space-y-4">
        {routes.map((route, index) => (
          ...
          >
            <div
              className={`flex h-8 w-8 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-full font-bold shadow-sm text-sm sm:text-base ${
                index === 0
            ...
            <div className="flex-1 min-w-0">
              <h4 className="truncate text-xs sm:text-sm font-semibold text-slate-900">
                {route.route}
              </h4>
              <p className="text-[10px] sm:text-[11px] font-medium text-blue-500 truncate mb-0.5">
              ...
              <p className="text-[10px] sm:text-xs text-slate-500 whitespace-nowrap overflow-hidden text-ellipsis mt-0.5">
                ...
                <span className="hidden sm:inline">
                  • Agency Cost: ₹
                  {Math.round(route.avgRevenue).toLocaleString()}
                </span>
              </p>
            </div>
            ...
            <div className="text-right shrink-0">
              <p className="text-sm sm:text-base font-bold text-emerald-600">
                ₹{Math.round(route.profit ?? 0).toLocaleString()}
              </p>
